...
function initTable() {
    $('#bootstrap-table').bootstrapTable({
        ...
        /*向服务器发送请求*/
        url: '/manageConfFiles/CurrentConfData/' + selectValue + '/query',          //请求后台的URL
        ...
        buttons: [{
            ...
            action: function (dialog) {
                ...
                $.ajax({
                    ...
                    url: '/manageConfFiles/CurrentConfData/' + selectValue + '/add'
                }).done(function (res) {
        ...
        buttons: [{
            ...
            action: function (dialog) {
                ...
                $.ajax({
                    ...
                    data: data,
                    contentType: 'application/json; charset=utf-8',
                    url: '/manageConfFiles/CurrentConfData/' + selectValue + '/update'
                }).done(function (res) {
                    if (!res.ret) {//校验出错，提示错误信息
            ...
            action: function (dialog) {
                select.forEach(function (ele) {
                    ...
                    $.ajax({
                        type: 'POST',
                        data: {id: id},
                        url: '/manageConfFiles/CurrentConfData/' + selectValue + '/delete'
                    });
                });
                dialog.close();
                $('#bootstrap-table').bootstrapTable('refresh');
...
function diff() {
    ...
    $.ajax({
        type: 'POST',
        url: '/manageConfFiles/CurrentConfData/' + selectValue + '/diff'
    }).done(function (res) {
        if (res.ret) {
            var data = [];
            ...
            BootstrapDialog.show({
                ...
                buttons: [{
                    ...
                    action: function (dialog) {
                        ...
                        $.ajax({
                            type: 'POST',
                            url: '/manageConfFiles/CurrentConfData/' + selectValue + '/publish'
                        }).done(function (res) {
                            console.log(res);
                            if (res.ret) {
                                dialog.close();
